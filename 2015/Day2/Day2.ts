...
order for more. They have a list of the dimensions (length l, width w, and height h) of each present, 
...
find the surface area of the box, which is 2*l*w + 2*w*h + 2*h*l. The elves also need a 
...
the smallest perimeter of any one face. Each present also requires a bow made out of ribbon as
 well; the feet of ribbon required for the perfect bow is equal to the cubic feet of volume of the present. 
...
(() => {
  ...
  const calculateNeededWrappingPapper = (
    length: number,
    width: number,
    height: number
  ): number => {
    const calc1 = length * width;
    const calc2 = width * height;
    const calc3 = height * length;

    const smallSide = Math.min(calc1, calc2, calc3);

    return 2 * calc1 + 2 * calc2 + 2 * calc3 + smallSide;
  };

  const calculateNeededRibbon = (
    length: number,
    width: number,
    height: number
  ): number => {
    ...
    return second * 2 + first * 2 + length * width * height;
  };

  const part1 = (dimensions: string): number => {
    const arr: number[] = [];
    dimensions.split("\n").forEach((dimension) => {
      const [l, w, h] = dimension.split("x");
      arr.push(
        calculateNeededWrappingPapper(parseInt(l), parseInt(w), parseInt(h))
      );
    });
  ...
  const part2 = (dimensions: string): number => {
    const arr: number[] = [];
    dimensions.split("\n").forEach((dimension) => {
      const [l, w, h] = dimension.split("x");
      arr.push(calculateNeededRibbon(parseInt(l), parseInt(w), parseInt(h)));
    });
